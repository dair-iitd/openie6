...
def main():
    predD = dict()
    sense_carb = open('models/traditional/sense_carb.tsv','w')
    test_sents = [s.replace(' ','') for s in open('data/test/test.txt','r').readlines()]
    test_sents = open('data/test/test.txt').readlines()
    testD = dict()
    ...
    for row in csv.reader(open('models/traditional/SenseOIE_Output.csv','r')):
        ...
        if row[0] == 'y_predicted':
            ...
            #     ipdb.set_trace()
            norm_sentence = sentence.replace(' ','')
    ...
    test_lines = open('data/test/carb/test.tsv','r').readlines()
    filtered_test = open('data/test/carb/sense_test.tsv','w')
    found_test = set()
